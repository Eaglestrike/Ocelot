...
public class MotionProfile {
    ...
    // Variables that are calculated based on arguments that are to be used by methods to calculate velocity, position, and acceleration for a given time
    ...
    public MotionProfile(double time, double initialPosition, double finalPosition, double initialVelocity, double finalVelocity) {
        ...
        if (finalPosition - initialPosition <= distancePerfect) {
            ...
            T1 = (-initialVelocity + Math.sqrt(initialVelocity * initialVelocity + RobotSettings.MAX_ACCELERATION * (finalPosition - initialPosition))) / RobotSettings.MAX_ACCELERATION;
            ...
            P1 = initialPosition + (initialVelocity * T0) + ((RobotSettings.MAX_ACCELERATION * Math.pow(T0, 2)) / 2);
        ...
        if ((isTriangular && time > timePeriodTwo) || (!isTriangular && time > timePeriodThree)) throw new IllegalArgumentException("Already completed");
    }
